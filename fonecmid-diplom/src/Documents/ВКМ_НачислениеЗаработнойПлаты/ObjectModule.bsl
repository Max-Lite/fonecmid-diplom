...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Запрос.Текст =
		...
		|ВЫБРАТЬ
		|	ВКМ_Удержания.НомерСтроки КАК НомерСтроки,
		|	ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.СуммаБаза КАК СуммаОсн
		...
		|	ЕСТЬNULL(ВТ_Основные.СуммаОсн, 0) КАК СуммаОсн,
		|	ЕСТЬNULL(ВТ_Дополнительные.СуммаДоп, 0) КАК СуммаДоп,
		|	ВКМ_НДФЛ.Значение КАК НДФЛ
		|ИЗ
		|	ВТ_Основные КАК ВТ_Основные
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Дополнительные КАК ВТ_Дополнительные
		|		ПО ВТ_Основные.НомерСтроки = ВТ_Дополнительные.НомерСтроки,
		|	Константа.ВКМ_НДФЛ КАК ВКМ_НДФЛ";
	...
	Пока Выборка.Следующий() Цикл
		...
		Движение.Сумма = (Выборка.СуммаОсн + Выборка.СуммаДоп) * Выборка.НДФЛ / 100;
	КонецЦикла;
